...
async function buildAll() {
  ...
  await esbuild({
    ...
    external: [
      ...
      "@napi-rs/canvas", "@napi-rs/canvas-*", "bcrypt", "argon2",
      ...
      "@prisma/client", "@mikro-orm/*", "@grpc/*", "@swc/*",
...
globalThis.__dirname = __bannerPath.dirname(globalThis.__filename);
  ...
  // Cover art for print PDF renders must sit next to the running process.
  try {
    await cp(path.resolve(artifactDir, "assets"), path.resolve(distDir, "assets"), { recursive: true });
  } catch (err) {
    console.warn("Could not copy assets/ into dist:", err);
  }
}
